...
> **History (2026-06-17/18):** the v3.1 task-level `AgentDispatcher` — which routed tasks through a `PMAgent` → `SchedulerAgent`/Prep hierarchy over a uniform `Agent` dispatch contract — was **removed** (resolution: keep-the-ideas, drop-the-framework). It was built and unit-tested but never constructed in production. `DecompositionService` was wired as a direct service (R2, `/breakdown`); the tool-validation seam was made load-bearing on the live **skills** path (R3, 2026-06-18 — per-tool param JSON-schema validation + caller-identity audit in `donna.skills.tool_registry.ToolRegistry`, fail-closed on bad args; the dead `agents/tool_registry.py` and the unused `AgentContext.db`/`tool_registry` fields were deleted). `config/agents.yaml` remains the live allowlist registry (challenger/research) behind the tool-lint safety check and admin UI. See `spec_v3.md §7.2` and [`2026-06-17-subagent-72-resolution-design.md`](../superpowers/specs/2026-06-17-subagent-72-resolution-design.md).
