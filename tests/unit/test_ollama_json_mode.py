...
class TestOllamaJsonMode:
    ...
    async def test_output_schema_sent_as_format(self) -> None:
        """Structured outputs: a JSON schema replaces the bare "json" format."""
        provider = OllamaProvider(base_url="http://fake:11434")
        schema = {
            "type": "object",
        ...
        with patch.object(provider, "_get_session", return_value=mock_session):
            await provider.complete(
                "test prompt", "test-model", json_mode=True, output_schema=schema
            )

        payload = mock_session.post.call_args[1]["json"]
        assert payload["format"] == schema

    async def test_output_schema_ignored_when_json_mode_false(self) -> None:
        provider = OllamaProvider(base_url="http://fake:11434")
        ...
        with patch.object(provider, "_get_session", return_value=mock_session):
            await provider.complete(
                "test prompt", "test-model", json_mode=False,
