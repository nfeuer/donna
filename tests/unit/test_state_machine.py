...
def config() -> StateMachineConfig:
    ...
    return StateMachineConfig(
        ...
        transitions=[
            TransitionEntry(**{"from": "backlog", "to": "scheduled", "trigger": "scheduler_assigns_slot", "side_effects": ["create_calendar_event"]}),
            TransitionEntry(**{"from": "scheduled", "to": "in_progress", "trigger": "user_starts", "side_effects": ["set_actual_start"]}),
            TransitionEntry(**{"from": "scheduled", "to": "backlog", "trigger": "user_cancels", "side_effects": ["delete_calendar_event", "increment_reschedule_count"]}),
            TransitionEntry(**{"from": "in_progress", "to": "done", "trigger": "user_completes", "side_effects": ["set_completed_at"]}),
            TransitionEntry(**{"from": "in_progress", "to": "blocked", "trigger": "blocker_reported", "side_effects": ["log_blocking_reason"]}),
            TransitionEntry(**{"from": "in_progress", "to": "scheduled", "trigger": "user_reschedules", "side_effects": ["increment_reschedule_count"]}),
            TransitionEntry(**{"from": "blocked", "to": "scheduled", "trigger": "blocker_resolved", "side_effects": ["find_next_available_slot"]}),
            TransitionEntry(**{"from": "blocked", "to": "cancelled", "trigger": "user_abandons", "side_effects": ["flag_dependent_tasks"]}),
            TransitionEntry(**{"from": "waiting_input", "to": "scheduled", "trigger": "info_provided", "side_effects": []}),
        ...
        invalid_transitions=[
            ...
            InvalidTransitionEntry(**{"from": "cancelled", "to": "*", "except": ["backlog"], "reason": "Must re-open to backlog first."}),
            InvalidTransitionEntry(**{"from": "done", "to": "scheduled", "reason": "Must go through in_progress first."}),
        ],
    )
