...
class TestHandleMessage:
    async def test_creates_session_if_none_active(
...
class TestEscalationApproval:
    async def test_handle_escalation_calls_claude(
...
class TestSessionPinning:
    async def test_pin_session(self, engine: ConversationEngine, mock_db: AsyncMock) -> None:
        mock_db.get_chat_session.return_value = MagicMock(
    ...
    async def test_unpin_session(self, engine: ConversationEngine, mock_db: AsyncMock) -> None:
        mock_db.get_chat_session.return_value = MagicMock(
            id="sess-1", user_id="nick", status="active",
        )
