...
- **Cloud LLM:** Claude API (claude-sonnet-4-6) — sole provider until local LLM hardware available
...
- **Spec follow-ups log.** Cross-slice follow-ups, deferred decisions, and known-but-accepted drifts live in `docs/superpowers/specs/followups.md`. When you finish a slice, scan your work for items that belong there and append an entry. This log does **not** replace updating the canonical spec — both must happen when behaviour diverges from the spec.
